...
#ifdef UDP_DEBUG2
#define UDP_DUMP2(x) printf("UDPSVR: "); x
#else
#define UDP_DUMP2(x)
#endif
 ...
 public:
  ...
  virtual void success() {
    ...
    case UDP_SERVER_SESSION_RECEIVING:
      //server->request_handler(this);   //ASV
...
UDPServerSessionContinuation* reservoir[600];  //ASV reservoir
...
void UDPServerSession::start_on()
{
  ...
  HandleCode h = inHost()->waitFor( start_timer_callback_window, ac, 0, inHost()->tie_breaking_seed ); //currently the starting time is 0
  handle_client(ssock);
...
void UDPServerSession::restart_timer()
{
  ...
  start_timer_ac_window = new ProtocolCallbackActivation(this);
  Activation ac (start_timer_ac_window);
  HandleCode h = inHost()->waitFor( start_timer_callback_window, ac, send_interval, inHost()->tie_breaking_seed ); //currently the starting time is 0
}
...
void UDPServerSession::request_received(UDPServerSessionContinuation* const cnt)
{
  ...
  assert(cnt->retval == (int)request_size); /* ASV */
...
void UDPServerSession::init_array(int size)
{
	...
	UDP_DUMP2(printf("Reservoir initialized (Size %i)\n",size));
	array_iter = 0;
}
...
void UDPServerSession::add_to_array(UDPServerSessionContinuation* const cnt)
{
	...
	{
		UDP_DUMP2(printf("*Reservoir is being processed. Request discarded\n"));

		// the server will close the discarded connection.
  		cnt->status = UDPServerSessionContinuation::UDP_SERVER_SESSION_CLOSING;
  		sm->close(cnt->client_socket, cnt);
	}
	else
	{
		...
		{
			UDP_DUMP2(printf("+++Inserting REQ in pos %i\n", array_iter));
...
void UDPServerSession::replace_in_array(UDPServerSessionContinuation* const cnt)
{
	...
	int randNmb = rand() % max_bound;

	if (randNmb < array_iter)
	{
		int pos = rand() % array_iter;
		UDP_DUMP(printf("Replacing request in position %i of the reservoir\n", pos));

		UDPServerSessionContinuation* const discarded_req = reservoir[pos];
		reservoir[pos] = cnt;

	  	// the server will close the discarded connection.
	  	discarded_req->status = UDPServerSessionContinuation::UDP_SERVER_SESSION_CLOSING;
	  	sm->close(discarded_req->client_socket, discarded_req);
	}
	else
	{
		UDP_DUMP(printf("New request won't be inserted in reservoir. Discarding\n", pos));
...
{
	// Not really emptying the array but as far as we are concerned this works as we want
	array_iter = 0;
	req_count = 0;
}
...
void UDPServerSession::process_elements()
{
